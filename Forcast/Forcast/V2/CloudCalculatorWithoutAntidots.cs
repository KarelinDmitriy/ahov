...
{
	public class CloudCalculatorWithoutAntidots : CloudCalculator
	{
		public CloudCalculatorWithoutAntidots(IEnumerable<BarrelV2> barrels, StorageData storageData, ActiveData activeData) : base(barrels, storageData, activeData)
		{
		}
		...
		protected override void NpaNopa()
		{
			...
			{
				npa[i] = StorageData.N * StorageData.a[i.Y] * (1 - StorageData.aa[i.Y])
				...
				nopa[i] = StorageData.No * (1 - StorageData.aao) * soapDelta
		...
		{
			new SawSapCalculator(StorageData, ActiveData, iv, true).Calculate();
		}
		...
		protected override void Qpa2(double kp)
		{
			...
			foreach (var i in Index.GenerateIndices(5, 2))
			{
				result[i] = koef * Barrels.Sum(x => x.D * x.D * er(x) * 4 / (x.Matter.ToksiDosesPorog()[i] * Constants.KadForWithoutDef[i.X]));
			}
			iv.Qpa2 = result;
		}
	}
